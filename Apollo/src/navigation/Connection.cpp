...
{
  apLog_Info((LOG_CHANNEL, "Connection::OnConnected", "Navigation connection " ApHandleFormat " opened", ApHandleType(hAp_)));
  return 1;
}
...
{
  AutoDelete(1);
  apLog_Info((LOG_CHANNEL, "Connection::OnClosed", "Navigation connection " ApHandleFormat " closed", ApHandleType(hAp_)));

  NavigationModuleInstance::Get()->removeConnection(apHandle());
