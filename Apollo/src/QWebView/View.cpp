...
void View::Create(int nLeft, int nTop, int nWidth, int nHeight)
{
  ...
  if (FAILED( WebKitCreateInstance(CLSID_QWebView, 0, IID_IQWebView, reinterpret_cast<void**>(&pQWebView_)) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " WebKitCreateInstance(CLSID_QWebView) failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebView_->setUIDelegate(this) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebView_->setUIDelegate() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebView_->setFrameLoadDelegate(this) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebView_->setFrameLoadDelegate() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebView_->setResourceLoadDelegate(this) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebView_->setResourceLoadDelegate() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebView_->setPolicyDelegate(this) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebView_->setPolicyDelegate() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebView_->QueryInterface(IID_IQWebViewPrivate, reinterpret_cast<void**>(&pQWebViewPrivate_)) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " QueryInterface(IID_IQWebViewPrivate) failed", ApHandlePrintf(hAp_)); }

  RECT r = { nLeft_, nTop_, nWidth_ + nLeft_, nHeight_ + nTop_ };
  if (FAILED( pQWebView_->initWithFrame(r, 0, 0) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebView_->initWithFrame() failed", ApHandlePrintf(hAp_)); }

  // -----------------------------

  if (FAILED( pQWebViewPrivate_->setFrameLoadDelegatePrivate(this) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebViewPrivate_->setFrameLoadDelegatePrivate() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebViewPrivate_->setTransparent(TRUE) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebViewPrivate_->setTransparent() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebViewPrivate_->setUsesLayeredWindow(TRUE) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebViewPrivate_->setUsesLayeredWindow() failed", ApHandlePrintf(hAp_)); }
  if (FAILED( pQWebViewPrivate_->viewWindow(reinterpret_cast<OLE_HANDLE*>(&hWnd_))) ) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebViewPrivate_->viewWindow() failed", ApHandlePrintf(hAp_)); }
  if (hWnd_ == NULL) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebViewPrivate_->viewWindow() did not return hWnd", ApHandlePrintf(hAp_)); }

  // -----------------------------

  if (FAILED( pQWebView_->mainFrame(&pWebFrame_) )) { throw ApException(LOG_CONTEXT, "" ApHandleFormat " pQWebView_->mainFrame() failed", ApHandlePrintf(hAp_)); }
...
void View::Destroy()
{
  ...
  if (pQWebViewPrivate_) {
    pQWebViewPrivate_->Release();
    pQWebViewPrivate_ = 0;
  }
...
HRESULT View::didStartProvisionalLoadForFrame(IQWebView* webView, IWebFrame* frame)
{
...
HRESULT View::didFinishDocumentLoadForFrame(IQWebView *webView, IWebFrame *frame)
{
...
HRESULT View::didFinishLoadForFrame(IQWebView* webView, IWebFrame* frame)
{
  String sUrl = GetUrlFrom(frame);
...
HRESULT View::didFailLoadingWithError(IQWebView *webView, unsigned long identifier, IWebError *error, IWebDataSource *dataSource)
{
  if (pQWebView_ == webView) {
...
HRESULT View::willSendRequest(IQWebView *webView, unsigned long identifier, IWebURLRequest *request, IWebURLResponse *redirectResponse, IWebDataSource *dataSource, IWebURLRequest **newRequest)
{
  int bChanged = 0;
...
HRESULT View::decidePolicyForNavigationAction(IQWebView *webView, IPropertyBag *actionInformation, IWebURLRequest *request, IWebFrame *frame, IWebPolicyDecisionListener *listener)
{
  int bHandled = 0;
  int bNavigate = 1;
...
HRESULT View::willPerformDragSourceAction(IQWebView*, WebDragSourceAction, LPPOINT, IDataObject*, IDataObject**)
{
  // Disable drag
  return S_OK;
}

HRESULT View::webViewAddMessageToConsole(IQWebView *webView, BSTR message, int lineNumber, BSTR url, BOOL isError)
{
...
HRESULT View::webViewReceivedFocus(IQWebView *webView)
{
  if (pQWebView_ == webView) {
...
HRESULT View::webViewLostFocus(IQWebView *webView, OLE_HANDLE loseFocusTo)
{
  if (pQWebView_ == webView) {
...
HRESULT View::webViewClosing(IQWebView *webView)
{
  // In View::Destroy after pQWebView_ = 0
...
//HRESULT View::willCloseFrame(IQWebView* webView, IWebFrame* frame)
//{
//  if (pTopLoadingFrame_ == frame) {
//    Msg_WebView_Event_Closing msg;
//    msg.hView = apHandle();
