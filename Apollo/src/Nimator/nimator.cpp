// ============================================================================
//
// Apollo
//
// ============================================================================

#include "Apollo.h"
#include "nimator.h"
#include "MsgAnimation.h"
#include "MsgUnitTest.h"
#include "Local.h"
#include "NimatorModule.h"

#if defined(WIN32)
BOOL APIENTRY DllMain(HANDLE hModule, DWORD dwReason, LPVOID lpReserved)
{
  AP_MEMORY_CHECK(dwReason);
  AP_UNUSED_ARG(hModule);
  AP_UNUSED_ARG(dwReason);
  AP_UNUSED_ARG(lpReserved);
  return TRUE;
}
#endif // defined(WIN32)

static AP_MODULE_INFO g_info = {
  sizeof(AP_MODULE_INFO),
  AP_MODULE_INTERFACE_FLAGS_CPP | AP_MODULE_INTERFACE_FLAGS_UTF8
  ,
  MODULE_NAME,
  "Nimator",
  "Nimator Module",
  "1",
  "Provides animations for various avatar types, specifically the famous config.xml, maybe more.",
  PROJECT_Author,
  PROJECT_Email,
  PROJECT_Copyright,
  PROJECT_HomepageUrl
};

NIMATOR_API AP_MODULE_INFO* Info(AP_MODULE_CALL* pModuleData)
{
  AP_UNUSED_ARG(pModuleData);
  return &g_info;
}

//----------------------------------------------------------

NIMATOR_API int Load(AP_MODULE_CALL* pModuleData)
{
  AP_UNUSED_ARG(pModuleData);

  int ok = 0;

  NimatorModuleInstance::Delete();
  if (NimatorModuleInstance::Get() != 0) { ok = NimatorModuleInstance::Get()->Init(); }

  return ok;
}

NIMATOR_API int UnLoad(AP_MODULE_CALL* pModuleData)
{
  AP_UNUSED_ARG(pModuleData);

  int ok = 1;

  if (NimatorModuleInstance::Get() != 0) {
    NimatorModuleInstance::Get()->Exit();
    NimatorModuleInstance::Delete();
  }

  return ok;
}
